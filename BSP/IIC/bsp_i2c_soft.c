...
{
	iic_pin->IIC_SDA_GPIO_PORT->MODER&=~(3<<(iic_pin->IIC_SDA_PIN_NUM*2));
	iic_pin->IIC_SDA_GPIO_PORT->MODER|= (0<<(iic_pin->IIC_SDA_PIN_NUM*2));	    
}
...
{
	iic_pin->IIC_SDA_GPIO_PORT->MODER&=~(3<<(iic_pin->IIC_SDA_PIN_NUM*2));
	iic_pin->IIC_SDA_GPIO_PORT->MODER|= (1<<(iic_pin->IIC_SDA_PIN_NUM*2)); 		
}
...
/**
  ...
  * @retval void
  */
void bsp_I2C_Init(IIC_PIN_Typedef *iic_pin, GPIO_TypeDef *gpio_scl, GPIO_TypeDef *gpio_sda, uint32_t scl_pinx, uint32_t sda_pinx, uint32_t scl_pin_num, uint32_t sda_pin_num)
{ 
