/**
  ...
  * @brief   Code for UART driver in STM32 series MCU	 
    ...
    @attention
			-# 本例程仅提供串口1、2、4、6的收发。其中1、2、4为串口空闲中断DMA双缓存接收，6为中断接收
				 用户视情况自行选择配置串口使用
      -# 本模块仅支持LL库
  ******************************************************************************
  * @attention
...
/**
  * @brief  串口6初始化
	* @param	fun					中断回调函数
 **/
void bsp_usart6_Init(usart_call_back fun)
{
...
void usart6_IQR_Service(void)
{
	...
	{
		LL_USART_ClearFlag_RXNE(USART6);
		Rx_buf = LL_USART_ReceiveData8(USART6);
		if(usart_manage_obj[6].call_back_f != NULL)
				usart_manage_obj[6].call_back_f(&Rx_buf, 1);
	}
}
