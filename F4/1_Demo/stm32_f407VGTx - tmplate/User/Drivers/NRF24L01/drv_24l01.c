...
uint8_t NRF24L01_Read_Buf(uint8_t reg,uint8_t *pBuf,uint8_t len)
{
  ...
  NRF_CS(0);          
  status=SPI_ReadWrite_8Byte(SPI1, reg);//发送寄存器值(位置),并读取状态值   	   
  for(u8_ctr=0;u8_ctr<len;u8_ctr++)pBuf[u8_ctr]=SPI_ReadWrite_8Byte(SPI1, 0XFF);//读出数据
  NRF_CS(1);
...
uint8_t NRF24L01_Write_Buf(uint8_t reg, uint8_t *pBuf, uint8_t len)
{
  uint8_t status,u8_ctr;	    
 	NRF_CS(0);          
  status = SPI_ReadWrite_8Byte(SPI1, reg);//发送寄存器值(位置),并读取状态值
  for(u8_ctr=0; u8_ctr<len; u8_ctr++)SPI_ReadWrite_8Byte(SPI1, *pBuf++); //写入数据	 
  NRF_CS(1);       
  return status;          //返回读到的状态值
}	
