...
unsigned short Read_W5500_SOCK_2Byte(SOCKET s, unsigned short reg)
{
	...
	HAL_GPIO_WritePin(W5500_SCS_GPIO_Port,W5500_SCS_Pin, GPIO_PIN_RESET);	//置W5500的SCS为低电平
	...
	HAL_GPIO_WritePin(W5500_SCS_GPIO_Port,W5500_SCS_Pin, GPIO_PIN_SET);//置W5500的SCS为高电平
	return i;//返回读取到的寄存器数据
}
...
{
	HAL_GPIO_WritePin(W5500_SCS_GPIO_Port,W5500_SCS_Pin, GPIO_PIN_RESET);//复位引脚拉低
	delay_ms(50);
	HAL_GPIO_WritePin(W5500_SCS_GPIO_Port,W5500_SCS_Pin, GPIO_PIN_SET);//复位引脚拉高
	delay_ms(200);
	while((Read_W5500_1Byte(PHYCFGR)&LINK)==0);//等待以太网连接完成
}


void drv_w5500_init(void)
{
	u8 i=0;
